...
public class SeedDataService implements ApplicationRunner {
  ...
  private Product createProduct(String name, String description, BigDecimal price, int stockQuantity,
    ...
    if (offer && offerPrice != null) {
      applyDefaultOfferSchedule(product);
    }
    product.setMrp(mrp);
  ...
  private void backfillLegacyOfferSchedules() {
    ...
    List<Product> productsToBackfill = productRepository.findAll().stream()
        ...
        .peek(product -> applyDefaultOfferSchedule(product, defaultStartDate, defaultEndDate))
        .toList();
  ...
  private void applyDefaultOfferSchedule(Product product) {
    applyDefaultOfferSchedule(
        product,
        LocalDate.now().minusDays(DEFAULT_OFFER_LOOKBACK_DAYS),
        LocalDate.now().plusDays(DEFAULT_OFFER_DURATION_DAYS)
    );
  }

  private void applyDefaultOfferSchedule(Product product, LocalDate defaultStartDate, LocalDate defaultEndDate) {
