      ...
      <div className="section-container">
        <div className="home-section-panel" data-surface="pearl">
          <div className="mb-8 flex flex-col gap-4 md:mb-10 md:flex-row md:items-end md:justify-between">
            ...
            <Link
              to="/products"
              className="inline-flex items-center gap-2 self-start rounded-full border border-[hsl(var(--surface-line))] bg-white/80 px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-accent hover:text-accent"
            >
              Browse all products
          ...
          <div className="grid gap-5 lg:grid-cols-[1.35fr_0.95fr]">
            <Link
              ...
              className="group relative min-h-[420px] overflow-hidden rounded-[32px] border border-white/75 bg-white/88 shadow-[0_32px_72px_-40px_hsl(var(--surface-shadow)/0.42)]"
            ...
            >
              ...
              <div className="relative flex h-full max-w-xl flex-col justify-between p-8 md:p-10 lg:max-w-[62%]">
                <div className="max-w-xl">
                  ...
                  <h3 className="mt-5 font-heading text-3xl font-bold leading-tight text-foreground md:text-5xl">
                    {leadProduct.name}
                  </h3>
                  <p className="mt-4 max-w-lg text-sm leading-6 text-foreground/70 md:text-base">
                    {leadProduct.short_description || leadProduct.description}
                  </p>
                </div>

                <div className="flex flex-col gap-4 border-t border-[hsl(var(--surface-line))] pt-5 md:flex-row md:items-end md:justify-between">
                  <div>
              ...
              {supportingProducts.map((product, index) => {
                ...
                return (
                  <Link
                    ...
                    className="group relative min-h-[200px] overflow-hidden rounded-[28px] border border-white/78 bg-white/86 shadow-[0_28px_62px_-38px_hsl(var(--surface-shadow)/0.38)]"
                  ...
                  >
                    ...
                    <div className="relative flex h-full max-w-[72%] flex-col justify-end p-6 text-foreground sm:max-w-[68%]">
                      <span className="mb-3 inline-flex w-fit rounded-full border border-[hsl(var(--surface-line))] bg-white/88 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-foreground/68 backdrop-blur">
                        {index === 0 ? "Fresh pick" : "Trending"}
                      </span>
                      <h3 className="font-heading text-2xl font-bold leading-tight">{product.name}</h3>
                      <div className="mt-3 flex items-center justify-between gap-3 text-sm text-foreground/62">
